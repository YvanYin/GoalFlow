...
class PDMScorer:
    ...
    def score_dac(
        ...
        self._calculate_drivable_area_compliance_lastpoints()
    ...
    def _calculate_drivable_traffic_compliance_lastpoints(self) -> None:
        """
        Re-implementation of nuPlan's drivable area compliance metric
        """
        drivable_traffic_compliance_scores = np.ones(self._num_proposals, dtype=np.float64)
        off_traffic_mask = self._ego_areas[:, -1, EgoAreaIndex.ONCOMING_TRAFFIC]
        drivable_traffic_compliance_scores[off_traffic_mask] = 0.0
        self._multi_metrics[MultiMetricIndex.DRIVING_DIRECTION] = drivable_traffic_compliance_scores

    def _calculate_drivable_area_compliance_lastpoints(self) -> None:
        """
        Re-implementation of nuPlan's drivable area compliance metric
        """
        drivable_area_compliance_scores = np.ones(self._num_proposals, dtype=np.float64)
        off_road_mask = self._ego_areas[:, -1, EgoAreaIndex.NON_DRIVABLE_AREA]
        drivable_area_compliance_scores[off_road_mask] = 0.0
        self._multi_metrics[MultiMetricIndex.DRIVABLE_AREA] = drivable_area_compliance_scores
