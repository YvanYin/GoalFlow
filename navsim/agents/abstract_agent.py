...
from navsim.common.dataclasses import AgentInput, Trajectory, SensorConfig, Scene, Annotations
...
class AbstractAgent(torch.nn.Module, ABC):
    ...
    def compute_trajectory(self, agent_input: AgentInput, scence: Scene, token=None) -> Trajectory:
        """
        ...
        for builder in self.get_feature_builders():
            features.update(builder.compute_features(agent_input, scence))
            # features.update(builder.compute_features(agent_input))

        for builder in self.get_target_builders():
            targets.update(builder.compute_targets(scence))
