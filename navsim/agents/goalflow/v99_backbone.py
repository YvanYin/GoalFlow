...
from vovnet.vovnet import build_fcos_vovnet_fpn_backbone_p6
from vovnet.layers.shape_spec import ShapeSpec


class V299Backbone(nn.Module):
    """
    ...
    def __init__(self, config):
        ...
        v299_cfg={'NORM':'BN','CONV_BODY':'V-99-eSE','FREEZE_AT':0,'IN_FEATURES':['stage2', 'stage3', 'stage4', 'stage5'], \
                'FPN_OUT_CHANNELS':256,'VOVNET_OUT_FEATURES':['stage2', 'stage3', 'stage4', 'stage5'],'FUSE_TYPE':'sum'}
        self.PIXEL_MEAN=[0.406,0.456,0.485]
        self.PIXEL_STD=[0.225,0.224,0.229]
        input_shape = ShapeSpec(channels=len(self.PIXEL_MEAN))
        self.image_encoder=build_fcos_vovnet_fpn_backbone_p6(v299_cfg,input_shape)
        if not config.v99_pretrained_path=='':
            ...
            self.image_encoder.load_state_dict(backbone_state_dict)
    ...
    def forward(self, image, lidar):
        ...
        # image_features=image_features[:,:,:,272:-272]
        # 256,1024
        # 256,480

        mean = torch.tensor(self.PIXEL_MEAN, dtype=torch.float32).view(1, 3, 1, 1).to(image_features)
        std = torch.tensor(self.PIXEL_STD, dtype=torch.float32).view(1, 3, 1, 1).to(image_features)

        image_features = (image_features - mean) / std


        # Generate an iterator for all the layers in the network that one can loop through.
        # image_layers = iter(self.image_encoder.pretrained.items())
        ...
        image_features=self.image_encoder(image_features)['p6']
        # Loop through the 4 blocks of the network.
        for i in range(4):
            
            # lidar_features=self.lidar_encoder(lidar_features)[-1]
