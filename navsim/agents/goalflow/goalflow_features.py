...
class GoalFlowFeatureBuilder(AbstractFeatureBuilder):
...
class GoalFlowTargetBuilder(AbstractTargetBuilder):
    def __init__(self, config: GoalFlowConfig):
        self._config = config
    ...
    def compute_targets(self, scene: Scene) -> Dict[str, torch.Tensor]:
        ...
        # drivable_area_map=self._compute_drivable_area_map(annotations,scene.map_api,ego_pose)
    ...
    def _compute_agent_targets(self, annotations: Annotations) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        def _xy_in_lidar(x: float, y: float, config: GoalFlowConfig) -> bool:
    ...
    def _compute_drivable_area_map(
        self, annotations: Annotations, map_api: AbstractMap, ego_pose: StateSE2
    ) -> torch.Tensor:
        """
        Creates sematic map in BEV
        ...
        drivable_area_map = np.zeros(self._config.bev_semantic_frame, dtype=np.int64)
        for label, (entity_type, layers) in self._config.drivable_area_classes.items():
            if entity_type == "polygon":
                entity_mask = self._compute_map_polygon_mask(map_api, ego_pose, layers)
            drivable_area_map[entity_mask] = label

        return torch.Tensor(drivable_area_map)
